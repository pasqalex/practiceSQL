...
- Комбинация `UNNEST` + `JOIN` для связи товаров и заказов.
...
- `WINDOW` — переиспользование оконного определения.
- `ROUND` + приведение к `DECIMAL` для корректных дробей.
- Расчёт метрик на уровне заказа, пользователя и дня.
...
- Использование `CROSS JOIN` для подстановки одиночных значений.
- Защита от деления на ноль через `NULLIF`.
- Осознанный выбор между `NOT EXISTS` и `LEFT JOIN ... IS NULL`.
